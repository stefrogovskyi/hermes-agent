...
cliches = [
    "delve", "seamless", "testament", "tapestry", "empowering", "elevate", 
    "beacon", "game-changer", "transformative", "fostering", "pivotal", 
    "vibrant", "holistic", "realm", "synergy", "unravel", "unlock", 
    "paramount", "cutting-edge", "spearhead", "robust", "revolutionize",
    "landscape", "thrive", "harness", "foster", "strive", "vital", "testament"
]

for word in cliches:
    m = re.findall(r'\b' + word + r's?\b', rewrite_text, re.IGNORECASE)
    if m:
        print(f"Match found: {word} -> {m}")

print("Em-dash '—':", rewrite_text.count("—"))
print("Hyphen/dash '--':", rewrite_text.count("--"))
