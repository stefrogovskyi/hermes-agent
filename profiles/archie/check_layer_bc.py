...
ai_cliches = [
    ...
    "moreover", "in summary", "testament", "beacon", "catalyst", "nexus", "synergy",
    "spearhead", "pivotal role", "crucial", "vital", "elevate", "unleash", "harness"
]

print("=== CHECKING AI CLICHÉS ===")
found_cliches = []
for word in ai_cliches:
    matches = re.findall(r'\b' + re.escape(word) + r'\b', rewrite_draft, re.IGNORECASE)
    if matches:
        found_cliches.append((word, len(matches)))

print("Found AI Cliches:", found_cliches)

print("\n=== LAYER C: STRUCTURAL & RHETORICAL AI TICKS ===")
# Check connective bridges
connective_bridges = [
    "that's why", "which is why", "this means that", "that's a sign of", 
    "it's important to note", "it's worth noting", "as a result"
]
found_bridges = []
for cb in connective_bridges:
    if cb in rewrite_draft.lower():
        found_bridges.append(cb)
print("Connective bridges:", found_bridges)

# Check contrastive negations ("X, not Y", "instead of Y", "rather than Y")
contrastive = re.findall(r'(\b[\w\s]+,\s+not\s+[\w\s]+\b|\b[\w\s]+\s+instead of\s+[\w\s]+\b|\b[\w\s]+\s+rather than\s+[\w\s]+\b)', rewrite_draft, re.IGNORECASE)
print("Contrastive negations found:")
for c in contrastive:
    print("  -", c.strip())

# Check for Not only X, but also Y
not_only = re.findall(r'not only.*but also', rewrite_draft, re.IGNORECASE)
print("Not only... but also:", not_only)
