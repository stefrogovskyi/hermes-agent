...
rewrite_words = clean_words(rewrite_text)

# n-gram overlap check (6+ consecutive words)
n = 6
orig_ngrams = set()
...
print("--- N-GRAM OVERLAP CHECK (6 words) ---")
found_overlaps = []
for i in range(len(rewrite_words) - n + 1):
    ngram = " ".join(rewrite_words[i:i+n])
    if ngram in orig_ngrams:
        found_overlaps.append(ngram)

print(f"Total 6-gram exact matches: {len(found_overlaps)}")
for match in found_overlaps:
    print("Match:", match)

# Let's also check variable n-gram lengths (e.g. 5, 6, 7, 8, etc.) or sliding window
