...
print(f"Meta title length: {len(meta_title)}")
print(f"Meta description length: {len(meta_description)}")

# Let's check keywords:
keywords = [
    "supply chain visibility 2025",
...
for kw in keywords:
    found = kw.lower() in (title + meta_title + meta_description + body_markdown).lower()
    print(f"Keyword '{kw}': {'FOUND' if found else 'MISSING'}")

# Check forbidden strings
forbidden = [
    "—", "--", "delve", "testament", "game-changer", "seamlessly", "in today's world",
    "it's important to note", "thrilled", "welcome", "magic", "peaks", "bravest dreams",
    "that's why", "which is why", "instead of"
]

for f in forbidden:
    if f in (title + meta_title + meta_description + body_markdown).lower():
        print(f"WARNING: Forbidden phrase found: '{f}'")

# Check hyphens with spaces
if re.search(r'\s+-\s+', title + meta_title + meta_description + body_markdown):
    print("WARNING: space-hyphen-space found")
