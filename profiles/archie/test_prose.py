...
body_prose_only = """Week 21 brings operational updates to the SeaRates platform, centered on warehouse management, tracking updates, and carrier integrations.
...
Ship schedule queries received targeted carrier updates. Schedule options for Seaboard Marine and Wan Hai support filtering by Points, Unifeeder offers filtering by Vessel, and Evergreen filters by Port.
...
prose_words = tokenize(body_prose_only)

n = 6
orig_ngrams = set(tuple(orig_words[i:i+n]) for i in range(len(orig_words)-n+1))
prose_ngrams = [(i, tuple(prose_words[i:i+n])) for i in range(len(prose_words)-n+1)]

matches = []
for idx, ng in prose_ngrams:
    if ng in orig_ngrams:
        matches.append(' '.join(ng))

print(f"Prose-only 6-gram matches count: {len(matches)}")
for m in set(matches):
    print(f"  Match: {m}")

# Also test 5-grams
n5 = 5
orig_ngrams5 = set(tuple(orig_words[i:i+n5]) for i in range(len(orig_words)-n5+1))
prose_ngrams5 = [(i, tuple(prose_words[i:i+n5])) for i in range(len(prose_words)-n5+1)]
matches5 = [' '.join(ng) for idx, ng in prose_ngrams5 if ng in orig_ngrams5]
print(f"Prose-only 5-gram matches count: {len(matches5)}")
for m in set(matches5):
    print(f"  5-gram Match: {m}")
