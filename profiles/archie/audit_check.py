...
orig_words = tokenize_clean(original_text)
draft_words = tokenize_clean(draft_text)

print("Checking n-grams from n=5 to 10:")
for n in range(5, 11):
    o_set = set(zip(*[orig_words[i:] for i in range(n)]))
    d_set = set(zip(*[draft_words[i:] for i in range(n)]))
    overlap = o_set.intersection(d_set)
    print(f"{n}-grams overlap count: {len(overlap)}")
    for g in overlap:
        print("  ", " ".join(g))


print("\nChecking 4-grams overlap:")
o_set = set(zip(*[orig_words[i:] for i in range(4)]))
d_set = set(zip(*[draft_words[i:] for i in range(4)]))
overlap = o_set.intersection(d_set)
print(f"4-grams overlap count: {len(overlap)}")
for g in overlap:
    print("  ", " ".join(g))
