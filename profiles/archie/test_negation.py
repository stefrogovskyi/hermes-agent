...
print("Checking every sentence for contrastive negation, 'not', 'instead', 'rather', 'without', 'rather than':")
for i, s in enumerate(sentences):
    s_clean = s.replace('\n', ' ').strip()
    if any(k in s_clean.lower() for k in ['not', 'instead', 'rather', 'without', 'never', 'no longer']):
        print(f"Sent {i+1}: {s_clean}")
