...
cliches = [
    "game-changer", "game changer", "fast-paced", "in today's", "in today’s",
    "delve", "testament", "seamless", "tapestry", "beacon", "vital role",
    "pivotal", "foster", "realm", "landscape", "underscore", "harness",
    "elevate", "unlock", "ever-evolving", "paradigm", "synergy", "pave the way",
    "paved by", "paved", "it is worth noting", "crucial", "imperative", "beacon",
    "deep dive", "revolutionize", "cutting-edge", "game-changing", "paramount",
    "holistic", "robust"
]

print("Scanning for clichés and AI markers...")
found = False
for c in cliches:
    matches = list(re.finditer(r'\b' + re.escape(c) + r'\b', draft_text, re.IGNORECASE))
    if matches:
        found = True
        print(f"Found '{c}': {len(matches)} time(s)")
        for m in matches:
            s = max(0, m.start() - 30)
            e = min(len(draft_text), m.end() + 30)
            print(f"   Context: '{draft_text[s:e].replace(chr(10), ' ')}'")

if not found:
    print("No specified clichés found.")

# Check for em dashes
em_dashes = re.findall(r'—|--|\u2014|\u2013', draft_text)
print("Em-dashes/en-dashes count:", len(em_dashes))
