...
# 1. Connectors
connectors = ["that's why", "which is why", "this is why", "furthermore", "moreover", "as a result", "in addition"]
found_conn = [c for c in connectors if c in draft_text.lower()]
print("Explicit connectors found:", found_conn)

# 2. Contrastive negation
negation_words = ["not", "instead", "rather than", "never", "no longer", "but rather"]
found_neg = [n for n in negation_words if re.search(r'\b' + n + r'\b', draft_text.lower())]
print("Contrastive negation words found:", found_neg)

# 3. Parallelism / twin sentences inspection
