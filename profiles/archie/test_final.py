...
# Test metadata lengths
lines = text.strip().split('\n')
t = lines[0].replace('Title: ', '')
mt = lines[1].replace('Meta-Title: ', '')
md = lines[2].replace('Meta-Description: ', '')

assert len(t) <= 60, f"Title length {len(t)} > 60"
assert len(mt) <= 60, f"Meta-Title length {len(mt)} > 60"
assert len(md) <= 155, f"Meta-Description length {len(md)} > 155"

# Test prohibited dashes
dashes = re.findall(r'[—–]|--', text)
assert len(dashes) == 0, f"Found dashes: {dashes}"
...
for kw in keywords:
    assert kw.lower() in text.lower(), f"Missing keyword: {kw}"
