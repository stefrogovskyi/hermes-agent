...
3. **Inbound Replies vs. Mass Cold Outreach**:
   ...
   33. **B2B Vendor Onboarding & Supplier Qualification Forms (Procurement Compliance)**:
        - When enterprise clients or multinational manufacturers submit vendor registration packages (e.g. New Vendor Setup Form `.xlsx`, Supplier Qualification Record `.pdf`):
          * **Legal Entity & Reg**: `Wemelogistics Ltd (trading as navo24)`, UK Company No. `14081751`, incorporated 03 May 2022, 1 Robin Hood House, Kingston Vale, London SW15 3AL.
          * **Classification**: `Service Provider` / `Non-Trade` (Logistics software, API, MCP). Never classify as manufacturer or physical goods distributor.
          * **Directors & Billing**: Oleksii Shatunov (CEO / Director), Stefan Rogovskiy (MD / Director). Finance contact: Stefan Rogovskiy (`billing@navo24.com`, `+44 7554 565992`).
          * **Bank Receiving Coordinates**: Revolut multi-currency IBAN `GB97 REVO 2301 6390 1552 22`, BIC `REVOGB21` (USD Intermediary BIC `CHASGB2L`). Secondary: HSBC UK (`GB44 HBUK 4005 2042 3045 46`).
          * **Compliance & Certifications**: For physical industrial standards (ISO 9001/14001, DOE, DOSH, SAMM), select `No`; in Others, specify `UK GDPR / DCSA Compliant / SOC 2 Type II aligned`. Social Accountability: `Yes`.
          * **Technical Execution**: Use `openpyxl` for Excel (preserving fonts and borders) and `pymupdf` (`fitz`) overlay with vector ovals for static PDFs without interactive AcroForm fields. Leave internal buyer assessment sections blank.
          * *(See `references/b2b-vendor-onboarding-and-supplier-qualification.md` for complete field mappings, tax status, and sample scripts).*
   - **Interactive Agent Kanban Board Rules (Vercel Deployments)**:
      ...
      34. **Sales Rep Google Sheets Tracker, Lead Scoring & Multi-Channel Sequence Integration**:
           - **Ad-Hoc Google Sheets Lead Scoring & Review Workflow**: When team members (e.g. Elena, Nikita) submit a Google Sheet with prospective leads, LinkedIn URLs, or outreach drafts for review:
             * *Access Troubleshooting*: If access returns `403 / The caller does not have permission`, instruct the user to share the sheet with `richard-bot@navo-sales-sheets.iam.gserviceaccount.com` (Editor/Viewer) or open link access.
             * *BCO / Furniture & Retail Importer Scoring (ICP 2)*: Evaluate against high-volume ocean container thresholds (>50–100+ TEU/month, multi-carrier visibility, Demurrage & Detention risk).
             * *Multi-Threading Persona Hierarchy*: Score contacts by influence level:
               - **Tier A+ (10/10 - L1 Target)**: VP of Supply Chain, Director of Logistics / Transportation, Ocean Freight Director.
               - **Tier A (8.5–9.5/10 - L2 Champion)**: Logistics/Import Manager, Supply Chain Operations Lead.
               - **Tier B+ (7.5–8.0/10 - L3 / User)**: Logistics Analyst, Coordinator, or C-level (President/CEO) if too high for initial cold pitch.
             * *Message Hook Quality Audit*: Check for factual accuracy (241 ocean carriers, 132 direct connectors vs outdated 234/239), specific percentage spikes/container counts, and low-barrier open questions. Proactively recommend a multi-threading touch sequence (Day 1 operational leader, Day 3-4 VP, Day 7 referral check).
             * *LinkedIn Execution Boundary & Rep Guidance*: When reps ask the agent to execute LinkedIn outreach directly, clarify channel boundaries: Richard sends direct email (`rich@navo24.com` via MS Graph) and WhatsApp (+44 7360 065904). For LinkedIn, server-side headless automation triggers rapid account bans; guide the rep to (a) manual personalized connection notes from their own profile for small batches (<30 contacts, 50-65% acceptance), (b) authorized LinkedIn sequence tools (Expandi/Waalaxy) via CSV, or (c) converting profiles to verified email outreach via Hunter/Snov/Apollo.
             * *ImportYeti BCO Lead Qualification (US Customs BoL)*: When evaluating ImportYeti search links (e.g. `q=furniture`, `shipmentsTotal=500`, `mostRecentShipment=6mo`):
           ...
           - **2026 Anti-AI & Humanized B2B Copywriting Standards (Natural Founder-Led Tone)**:
             ...
             * **Name Normalization & Proper Title Case**: Always normalize recipient names to Title Case (`Emad`, `Marc`, `Ferdinand`) stripping all-caps raw logs and non-name strings before generating greetings.
             - **Carrier Count & Multimodal Precision (Live Sync Mandate)**:
               * Navo24 TrackingMCP covers **241 ocean carriers** (238 independent lines + 3 sub-brands/feeders, with **132 direct connectors**, 109 fallback/partner connectors, and 186 SCACs) and **97 global airlines / air cargo carriers** in AirCargo AWB tracking.
               * SchedulesMCP covers 60+ ocean carriers across 5,000+ lanes with 72,000+ live sailings.
               * **Daily Live Verification Mandate**: Daily website scanner (`scan_navo24_website.py` / `30 3 * * *` UTC) must poll the live registry endpoint `https://trackingmcp.com/carriers.json` directly. If carrier counts or direct connector totals update, synchronize immediately into `MEMORY.md`, email outreach templates, and sales documentation without delay.
             ...
             * **Lifecycle Notification Hygiene (Disabling Startup/Restart Alerts)**:
               - To permanently suppress operator-flavored Telegram restart alerts (`♻️ Gateway online — Richard Marlowe is back and ready`), NEVER attempt to edit `config.yaml` directly (which is rejected by Hermes config security guards).
               - Instead, use the Hermes CLI with the specific profile flag:
                 ```bash
                 hermes --profile richard config set platforms.telegram.gateway_restart_notification false
                 ```
               - This sets `gateway_restart_notification: false` under `platforms.telegram` in `/opt/hermes/profiles/richard/config.yaml` and suppresses all gateway online, restart, and shutdown notifications on Telegram.
             ...
             * **Internal Team Communication Style & No Meta-Preamble Mandate**:
               - When drafting internal team updates or messages to Stefan/colleagues, use an ultra-concise, conversational, relaxed peer-to-peer tone (e.g. 'Такс, собрал...', short bullets, zero corporate fluff). Never use emotional adjectives or demographic labels.
               - **Strict Zero Meta-Preamble**: When the user asks for a message or draft, omit introductory meta-sentences that announce the topic ("Остались ровно два вопроса:", "Вот готовый текст:"). Output the substantive content directly.
               - *(See `references/searates-archive-segmentation-and-in-transit-billing-rules.md` for archive distribution across reps, month-boundary billing rules, and in-transit operational event logic).*
   ...
   *(See `references/4-touch-sequence-pattern.md` for full sequence details, `references/pipeline-segregation-and-batch-outreach-rules.md` for CRM pipeline segregation and multi-batch campaign standards, `references/cross-border-tax-residency-and-hmrc-cor.md` for UK HMRC CoR and Form DGT tax workflows, `references/19-sources-daily-outreach-engine.md` for the complete 19-source lead ingestion pipeline, `references/inbound-triage-and-testimonials-workflow.md` for verbatim draft approval, silent watchdog semantics, full untruncated triage & team testimonials loop, `references/whatsapp-and-voice-telephony-pipeline.md` for WhatsApp gateway, voice PTT, Twilio outbound calling & full-duplex Voice AI streaming (Twilio Media Streams + OpenAI Realtime GA), `references/competitor-radar-and-weekly-sales-loops.md` for weekly testimonial loops, competitor radar 3-tier rules & Twilio voice pipeline, `references/telegram-team-access-and-isolation.md` for sales team colleague onboarding and session isolation, `references/25-sources-audit-and-scrapers.md` for the complete 25-source technical audit and scrapers, `references/b2b-contract-and-kanban-rules.md` for contract drafting and Vercel Kanban UI rules, `references/google-sheets-sales-tracker-and-crm.md` for sales representative Google Sheets tracker setup and gspread sync, and `references/technical-api-inquiries-and-schedules.md` for Tracking & Schedules REST API inquiry patterns, SeaRates comparison, and billing rules, and `references/technical-trial-feedback-and-escalation-playbook.md` for zero-fabrication escalation, gateway error diagnostics, and product pricing separation.)*
   ...
   - **Multi-Source Diversity & Strict Regional Filtering Mandate**:
     ...
     * **Full 5-Product Suite Inclusion**: Every cold outreach email body MUST list the full 5-component portfolio:
       1. `Tracking API (Ocean & Air)` (241 ocean carriers, 132 direct connectors, DCSA events, observed ETAs, D&D free-time)
   ...
   - Target Audience Scope (BCOs, Trading Conglomerates & Shippers First):
     ...
     * *(See `references/navo24-icp-and-sales-handbook.md` for the complete 4-segment ICP matrix, BANT discovery questions, and objection handling playbook).*
...
- **CRITICAL: The Meta-Preamble & Conversational Filler Trap**:
  * **The Trap**: When the user asks "Напиши коротко скажи какие еще два вопроса остались" or "дай готовый ответ", beginning the response with a meta-sentence announcing the topic (e.g., "Остались ровно два вопроса:", "Вот готовый текст для отправки:", "Ниже приведен ответ:") triggers extreme user frustration.
  * **Rule**: Output the requested content DIRECTLY and IMMEDIATELY without any preliminary topic sentence, preamble, or decorative wrapping. If the user asks for the two questions, the very first line of output must be the first question or their exact requested phrasing ("Спасибо за ответы! Значит осталось два вопроса:").
  * **No Emotional Labels or Generalizations**: In team escalations, never use emotional qualifiers ("очень жесткий", "злой клиент") or demographic/nationality markers ("индус"). Use clean, dry, professional facts (booking numbers, container IDs, error codes, explicit questions).
...
- **Outreach Monopoly Backfill Trap & Chronic Source Starvation**:
  ...
  * **Strict Hard Cap & Zero Backfilling**:
    ...
    3. Persistent Verified Multi-Source Cache: Maintain pre-verified pools in `/opt/hermes/profiles/richard/cache/multi_source_pools.json` across all 19 sources so runs maintain exact 5-per-source diversity without scraper degradation.
    4. Pre-Flight Dry Run: Always verify engine balance via `python daily_online_outreach_engine.py --dry-run` to confirm the exact 19x5 distribution before automated cron execution.
    5. Two-Step Directory Enrichment: Member directories (WCA, JCtrans, CIFFA, FIATA, Freightnet, ThomasNet) rarely publish direct personal executive emails on open pages. To prevent zero-yield runs, pair directory scraping (Company Name + Domain) with B2B email enrichment APIs (Prospeo, Hunter.io, Snov.io) targeting specific job titles (`VP Supply Chain`, `Director Ocean Freight`, `Head of Logistics`), validated by MX checks.
