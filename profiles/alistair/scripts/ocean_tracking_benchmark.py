...
SEARATES_API_KEY = os.getenv("SEARATES_API_KEY", "K-DD37901D-AF29-4CEE-A629-97D576E608AF")
SEARATES_ENDPOINT = "https://tracking.searates.com/tracking"

NAVO_API_KEY = os.getenv("NAVO_API_KEY", "tmcp_039ceee30bfbba0bf315726730c325e5d3a449768c4b230e")
...
def create_excel_report(track_id, sr_res, filepath):
    ...
    is_ok = (status_str.lower() == 'success' or status_str.lower() == 'ok')
    quota_val = sr_meta.get('api_calls', {}).get('remaining', 'N/A')
    
    findings = [
        (1, "SeaRates API Access", f"Status: {status_str.upper()}, Message: '{msg_str}'"),
        (2, "Container Granularity", f"SeaRates returned {len(sr_containers)} container(s) for track ID {track_id}." if is_ok else f"No containers returned due to API error ({msg_str})."),
        (3, "DSSA Error Handling", "DSSA errors suppressed in benchmark; focused on real milestone parity."),
        (4, "Navo Staging Status", "Navo API staging endpoint check executed."),
        (5, "API Quota Remaining", f"{quota_val} calls remaining on SeaRates key." if is_ok else "N/A (Key Invalid/Expired)")
    ]
    ...
    if sr_containers:
        ...
        ws2.append([1, "No events returned", "-", "-", "-", "-", "-", "-", "-", "-", "No", f"SeaRates returned status '{status_str}' ({msg_str})"])
        ws2.cell(ws2.max_row, 11).fill = fill_match_no
    ...
    if sr_containers:
        ...
        ws3.append(["SeaRates", "N/A", "N/A", status_str.upper(), 0, "False"])
    ...
    ws4.append(["API Quota Remaining", str(quota_val), "N/A", "Quota tracking."])
...
def main():
    ...
    status_str = sr_res.get('status', 'unknown')
    msg_str = sr_res.get('message', '')
    is_ok = (status_str.lower() in ('success', 'ok'))
    
    sr_meta = sr_res.get('data', {}).get('metadata', {})
    remaining_calls = sr_meta.get('api_calls', {}).get('remaining', 'N/A')
    
    if is_ok:
        api_status_line = f"✅ <b>SeaRates API Status:</b> LIVE & ACTIVE (Остаток лимитов: {remaining_calls})"
        overview_line = "• <b>Overview:</b> Прямой ответ SeaRates API получен успешно"
    else:
        api_status_line = f"❌ <b>SeaRates API Status:</b> ERROR ({msg_str or status_str.upper()})"
        overview_line = f"• <b>Overview:</b> Ошибка запроса к SeaRates API ({msg_str or status_str})"
    ...
    summary_text = (
        ...
        f"{api_status_line}\n\n"
        f"<b>Результаты аудита (5 вкладок):</b>\n"
        f"{overview_line}\n"
        f"• <b>Event Comparison:</b> Пособытийный аудит вех\n"
        f"• <b>Container Timestamps:</b> Валидация таймлайнов контейнеров\n"
        f"• <b>Structure & Metadata:</b> Проверка квот и метаданных\n"
        f"• <b>Route & Geometry:</b> Геолокации и морские маршруты\n\n"
        f"📎 Сформированный Excel-отчет подготовлен.\n"
        f"MEDIA:{filepath}"
    )
