...
## Pitfalls & Safeguards
- **`id` Parameter Type Mismatch (`got float64`):** When querying jobs or items by numeric ID (e.g. `upwork find_jobs get -p id="2095241986892776518"`), the CLI argument parser parses bare numeric strings as floating-point numbers (`float64`), which causes the MCP server to reject the call (`id must be a string, got float64`). **Fix:** Always pass parameters using the full JSON envelope: `upwork find_jobs --json '{"action":"get","params":{"id":"<JOB_ID_STRING>"}}'`.
