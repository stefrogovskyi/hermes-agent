...
def check_and_update():
    ...
    try:
        ...
        res_fetch = subprocess.run(["git", "-C", APP_DIR, "fetch", "origin"], capture_output=True, text=True, timeout=30)
        ...
        res_pull = subprocess.run(["git", "-C", APP_DIR, "pull", "--ff-only"], capture_output=True, text=True, timeout=45)
        if res_pull.returncode != 0:
            log(f"Git pull failed: {res_pull.stderr.strip()}")
            return

        log("Building OpenClaw 2.0 (npm run build)...")
        res_build = subprocess.run(["npm", "--prefix", APP_DIR, "run", "build"], capture_output=True, text=True, timeout=120)
        if res_build.returncode != 0:
            log(f"Build failed: {res_build.stderr[:200]}")
            return

        log("Restarting openclaw.service...")
        res_restart = subprocess.run(["systemctl", "restart", SERVICE_NAME], capture_output=True, text=True, timeout=20)
